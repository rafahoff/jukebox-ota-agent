...
public sealed class SqliteOtaPolicyProvider : IOtaPolicyProvider
{
    ...
    public OtaCheckPolicy GetPolicy(OtaAgentConfig config)
    {
        ...
        {
            return BuildPolicyFromConfig(config);
        }
    ...
    public static OtaCheckPolicy BuildPolicyFromConfig(OtaAgentConfig config)
    {
        var defaults = OtaCheckPolicy.Default;
        var enabled = config.OtaCheckEnabled ?? defaults.Enabled;
        var intervalMinutes = config.OtaCheckIntervalMinutes is int minutes
            ? OtaCheckPolicy.SnapIntervalMinutes(minutes)
            : defaults.IntervalMinutes;
        var windowStart = ParseConfigTime(config.OtaCheckWindowStart, defaults.WindowStart);
        var windowEnd = ParseConfigTime(config.OtaCheckWindowEnd, defaults.WindowEnd);
        return new OtaCheckPolicy(enabled, intervalMinutes, windowStart, windowEnd);
    }

    private static TimeOnly ParseConfigTime(string? raw, TimeOnly defaultValue) =>
        !string.IsNullOrWhiteSpace(raw) &&
        TimeOnly.TryParseExact(raw.Trim(), "HH:mm", CultureInfo.InvariantCulture, DateTimeStyles.None, out var parsed)
            ? parsed
            : defaultValue;
